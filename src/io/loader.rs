...
mod test {
    ...
    pub fn load_data_url() {
        ...
        let parsed_data_url: Texture2D = Texture2D::deserialize(
            data_url,
            RawAssets::new().insert(data_url, loaded.remove(data_url).unwrap()),
        )
        .unwrap();
        let loaded_image: Texture2D = loaded.deserialize(".png").unwrap();

        assert_eq!(loaded_data_url, parsed_data_url);
