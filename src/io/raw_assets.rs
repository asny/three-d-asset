use crate::{io::DeserializeAsset, Error, Result};
...
impl RawAssets {
    ...
    /// ```
    pub fn deserialize_asset<T: DeserializeAsset>(&mut self, path: impl AsRef<Path>) -> Result<T> {
        T::deserialize_asset(path, self)
    }
