...
pub fn deserialize_ply(raw_assets: &mut RawAssets, path: &PathBuf) -> Result<Scene> {
    ...
    let has_uchar_colors = first_vertex.map_or(false, |v| v.get_uchar("red").is_some());

    // check for gaussian splat fields, including color, scale, rotation, opacity, and spherical harmonic coefficients
    let has_gs_fields = first_vertex.map_or(false, |v| {
    ...
    for vertex in vertices {
        ...
        if has_uchar_colors {
            ...
            // Convert SH to linear color ( for preview only)
            ...
            // rotation
            let rot_x = vertex.get_float("rot_0").unwrap_or(0.0);
            let rot_y = vertex.get_float("rot_1").unwrap_or(0.0);
            let rot_z = vertex.get_float("rot_2").unwrap_or(0.0);
            let rot_w = vertex.get_float("rot_3").unwrap_or(1.0);
...
mod test {
    ...
    pub fn deserialize_gaussian_ply() {
        let _point_cloud: crate::PointCloud = crate::io::RawAssets::new()
            .insert(
